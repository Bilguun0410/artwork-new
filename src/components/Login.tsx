...
const Login = ({}: Props) => {
    ...
    const main = () => {
        switch (render) { 
            case 'register': return <Register setRender={setRender} />;
            default: return(
                <div className='col-span-1 h-full py-[77px] pl-[169px] pr-[75px]'>
                    <div className='rounded-[40px] bg-white shadow-[0_15px_40px_0px_rgba(0,0,0,0.15)] flex flex-col gap-32 w-full h-full p-[52px] pb-[150px]'>
                        ...
                        <div className='flex flex-col w-full gap-12'>
                            ...
                            <div className="grid w-full mx-auto items-center gap-1.5">
                                <Button onClick={handleLogin} type='button' className='bg-[#0C21C1] rounded-[32px] hover:bg-[#0C21C1]/75 py-8'>
                                    {loading && <Loader className='2-5 h-5 animate-spin' /> || 'Нэвтрэх'}
                                </Button>
                                <div className='text-xs w-full flex items-center justify-center'>
                                    <button type='button' onClick={()=> setRender('register')} className='underline text-center'>Бүртгүүлэх</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    return (
        <div className='grid grid-cols-2 xl:py-0 p-5 w-full h-full'>
            {main()}
            <AuroraBackground className='h-full'>
                <div className='col-span-1 rounded-2xl bg-gradient-to-tr from-[#000842] to-[#000842]/80 h-full flex-col flex'>
